  ...
  repeat with q = 1  to 1040 then
    ...
    if member("lightImage").getPixel(q-1,c-1) = 0 then
      repeat while stp = 0 then
        ...
        if d > 19 then
          exit repeat
        else if member("layer"&string(d)).image.getPixel(pnt+point(-1,-1)) <> color(255, 255, 255) then
          ...
          if d > 0 then
            repeat with dir in [point(-1, 0), point(0,-1), point(1,0), point(0,1)]then
              if member("layer"&string(d)).image.getPixel(pnt+point(-1,-1)+dir) <> color(255, 255, 255) then
