...
global gLEVEL, gLOprops, gLoadedName, gLOADPATH, massRenderSelectL
...
on exitFrame me
  ...
  if _key.keyPressed("A") and _movie.window.sizeState <> #minimized then
    ...
    massRenderSelectL = []
  else  if (_key.keyPressed(48))or(_key.keyPressed("1")) and _movie.window.sizeState <> #minimized then
    _movie.go(9)
  end if
