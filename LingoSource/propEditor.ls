...
on exitFrame me
  ...
  if _key.keyPressed(SPACE) then 
    ...
    if _key.keyPressed("T")then
      gPEprops.propStretchX = 1
      gPEprops.propStretchY = 1
    end if
    if _key.keyPressed("R")then
      gPEprops.propStretchX = 1
      gPEprops.propStretchY = 1
      gPEprops.propFlipX = 1
      gPEprops.propFlipY = 1
  ...
  if(gPEprops.props.count > 0)and((_key.keyPressed("V"))or(_key.keyPressed("B"))or(_key.keyPressed("M")) )then
  ...
  if(closestProp > 0)then
    ...
    else  if _key.keyPressed("B") then
    ...
    else if _key.keyPressed("M") then
    ...
    if(_key.keyPressed("U"))then
      peFreeQuad[1] = _mouse.mouseLoc - qd[1]
    else if(_key.keyPressed("I"))then
      peFreeQuad[2] = _mouse.mouseLoc - qd[2]
    else if(_key.keyPressed("O"))then
      peFreeQuad[3] = _mouse.mouseLoc - qd[3]
    else if(_key.keyPressed("P"))then
      peFreeQuad[4] = _mouse.mouseLoc - qd[4]
    else if(_key.keyPressed("K")) or (_key.keyPressed("R")) then
  ...
  if((gProps[gPEprops.pmPos.locH].prps[gPEprops.pmPos.locV].tp = "rope"))then
    ...
    if(editSettingsProp > 0)or(_key.keyPressed("M"))then
      mouseStill = 0
    end if
...
on checkKey(key)
  rtrn = 0
  gPEProps.keys[symbol(key)] = _key.keyPressed(key)
